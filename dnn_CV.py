...
class Dnn:
    ...
    def __init__(self):
        ...
        self.__load_train_dataset = False
        self.__load_test_dataset = False
        ...
        self._train_feature_size = None
        self._train_label_size = None
        self._test_dataset_size = None
        self._test_feature_size = None
        self._test_label_size = None
    ...
    def __del__(self):
        ...
        del self.__load_train_dataset
        del self.__load_test_dataset
        ...
        del self._train_dataset_size 
        del self._train_feature_size
        del self._train_label_size
        del self._test_dataset_size
        del self._test_feature_size
        del self._test_label_size
    ...
    def load_train_dataset(self, train_data):
        """
            속성들 리스트와 라벨 리스트를 가지는 튜플을 가지고 생성
            :train_data: tuple
    ...
    def load_test_dataset(self, test_data):
        
        self.__test_load_dataset = True
        self._test_dataset = Dataset()
        self._test_dataset = input_data.load_test_dataset(self._test_dataset,test_data)
        self._test_dataset_size = self._test_dataset.num_examples
        self._test_feature_size = len(test_data[0])
        self._test_label_size = 1
    ...
    def train(self, hnodes_num, dropout_layers=None, keep_prob=1.0, model_save_path='./DNN_Models/model'):
        ...
        if not self.__train_load_dataset:
        ...
        # set train count
        if self._train_dataset_size <= self._batch_size:
            train_count = 1
        else:
            if (self._train_dataset_size % self._batch_size) == 0:
                train_count = int(self._train_dataset_size / self._batch_size)
            else:
                train_count = int(self._train_dataset_size / self._batch_size) + 1
        ...
        with tf.Session() as sess:
            ...
            for step in range(self._epoch):
                ...
                self._train_dataset.reset_batch()
                if (step > 0) and (step % 20 == 0):
                    lr *= 0.1
                for i in range(train_count):
                    batch_x, batch_y = self._train_dataset.next_batch(
                        self._batch_size)
                    sess.run(
                        train_step,
    ...
    def eval(self, model_path):
        ...
        :return: list
        """
        if not self.__test_load_dataset:
            raise RuntimeError("Please Load Dataset by 'load_test_dataset(path).'")
        ...
        if self._test_dataset_size <= self._batch_size:
            test_count = 1
        else:
            if (self._test_dataset_size % self._batch_size) == 0:
                test_count = int(self._test_dataset_size / self._batch_size)
            else:
                test_count = int(self._test_dataset_size / self._batch_size) + 1

        # test
        self._test_dataset.reset_batch()
        for i in range(test_count):
            batch_x, batch_y = self._test_dataset.next_batch(self._batch_size)
            cost_ = sess.run(
                cost,
